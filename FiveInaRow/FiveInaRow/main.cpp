...
void clearStartButton() {
    ...
    fillrectangle(650, 270, 800, 350);  // 清除“VS启动”按钮区域
}
// 初始化游戏
void initGame() {
    // 创建界面
...
/*void calculateScore() {
    ...
    // 遍历棋盘上的每个空白位置
    for (int i = 1; i <= 15; i++) { // 使用 i 替代 row
        for (int j = 1; j <= 15; j++) { // 使用 j 替代 col
            // 仅计算空白点
            if (ChessBox[i - 1][j - 1] == 0) { // 由于数组从 0 开始，棋盘的坐标从 1 开始，所以要做减法
                ...
                for (int k = 0; k < 4; k++) {
                    ...
                    for (int n = 1; n <= 4; n++) {
                        ...
                        int ny = j + n * x;
                        if (nx >= 1 && nx <= 15 && ny >= 1 && ny <= 15) {
                            if (ChessBox[nx - 1][ny - 1] == 2) botNum++; // AI棋子
                            else if (ChessBox[nx - 1][ny - 1] == 0) emptyNum++; // 空位
                            else break;
                        }
                        else break;
    ...
    while (1) {
       ...
       if (Msg.uMsg == WM_LBUTTONDOWN) {
        ...
        while (flag % 2 && over == 0 && judgee == 0) {
            ...
            do {
                // 选择一个在棋盘交点上的位置
                MapX = ChessX = (rand() % 15 + 1);  // x坐标为 40, 80, ..., 600
                MapY = ChessY = (rand() % 15 + 1);  // y坐标为 40, 80, ..., 600
            } while (ChessBox[ChessX - 1][ChessY - 1] != 0);

            solidcircle(40 * ChessX, 40 * ChessY, 18);
            ChessBox[MapX - 1][MapY - 1] = 2;
            flag++;
            judgee = 1;
        }
        ...
        while (judgee) {
            ...
            if (judge(MapX, MapY)) {
                if (flag % 2 == 1) {
                    ...
                    MessageBox(NULL, "入机2白棋胜利", "游戏结束", MB_OK);
                }
                over = 1;
                break;  // 结束游戏
            }
    ...
    void PlayGameyl();{
        ...
        while (1) {
            ...
            while (flag % 2) {
                ...
                do {
                    // 选择一个在棋盘交点上的位置
                    ChessX = (rand() % 15 + 1);  // x坐标为 40, 80, ..., 600
                    ChessY = (rand() % 15 + 1);  // y坐标为 40, 80, ..., 600
                } while (ChessBox[ChessX - 1][ChessY - 1] != 0);
                solidcircle(40 * ChessX, 40 * ChessY, 18);
                ChessBox[ChessX - 1][ChessY - 1] = 2;
            }
            // 判断胜利
            if (judge(MapX, MapY)) {
                if (flag % 2 == 1) {
...
int main() {
   ...
   // MOUSEMSG xuanze;
    //xuanze = GetMouseMsg();
    //int mode;
   /* if (xuanze.uMsg == WM_LBUTTONDOWN) {
        if (xuanze.x >= 650 && xuanze.x <= 750 && xuanze.y >= 280 && xuanze.y <= 320 && !gameStarted) {
            // 点击了“VS启动”按钮，开始游戏
            gameStarted = 1;
            cleardevice();
            initGame1();
        ...
        if (xuanze.x >= 650 && xuanze.x <= 750 && xuanze.y >= 480 && xuanze.y <= 520 && !gameStarted) {
            // 点击了“AI启动”按钮，开始游戏
            gameStarted = 1;
            cleardevice();
            initGame2();
